...
<script type="text/javascript">
    function showWearAttributes(id, sizes, colors) {
        ...
        sizes.forEach(function (size) {
            content += showSizeElement(size);
        });
        content += '<br/>';
        content += 'Colors:';
        colors.forEach(function (color) {
            content += showColorElement(color);
        });
...
<div class="titsonfire-more-section">
    <div class="titsonfire-section-title"><a href="${commoditiesUrl}">ARCHIVE</a>&#160;${currentType.name}</div>
    <c:if test="${not empty commodities}">
        <div class="titsonfire-card-container mdl-grid">
            <c:forEach items="${commodities}" var="commodity">
                <div class="mdl-cell mdl-cell--3-col mdl-cell--4-col-tablet mdl-cell--4-col-phone mdl-card mdl-shadow--3dp">
                    <div class="mdl-card__media">
                        <a href="${showCommodityUrl}/${commodity.id}"><img class="article-image"
                                                                           src="${commodity.images[0]}" border="0"
                                                                           alt=""/></a>
                    </div>
