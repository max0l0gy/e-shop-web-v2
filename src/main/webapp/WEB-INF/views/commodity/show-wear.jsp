...
<script type="text/javascript">
    ...
    function showColors(el) {
        ...
        var colors = [];
        BRANCHES.forEach(function (value) {
            if (value.sizes[0].value === size) {
                colors.push(value.colors[0].value);
            }
        });
        colors.sort();
        colors.reverse();
    ...
    function showSizes() {
        ...
        var uniqSizes = [];
        BRANCHES.forEach(function (value) {
            if (!uniqSizes.includes(value.sizes[0].value)) {
                uniqSizes.push(value.sizes[0].value)
            }
        });
        uniqSizes.sort();
        uniqSizes.reverse();
        ...
        for (var i = 0; i < objJson.length; i++) {
            ...
            br.attributes.forEach(function (attribute) {
                if (attribute.name == "size") {
                    sizes.push({id: attribute.name, value: attribute.value});
                }
                if (attribute.name == "color") {
                    colors.push({id: attribute.name, value: attribute.value});
                }
            });
            var branch = {
                id: br.id,
...
<div class="titsonfire-more-section">
    <div class="titsonfire-section-title">${commodity.type.name}&#160;<b>${commodity.name}</b></div>
    <div class="titsonfire-card-container mdl-grid">
                ...
                <div class="images-navi mdl-grid">
                    <c:forEach items="${commodity.images}" var="image" varStatus="loop">
                        <c:if test="${loop.index==0}">
                            <img id="img-nav" src="${image}" onClick="mark(this, '${image}');"
                                 class="circleImgSelection"/>
                        </c:if>
                        <c:if test="${loop.index>0}">
                            <img id="img-nav" src="${image}" onClick="mark(this, '${image}');"
                                 class="circleImgUnselected"/>
                        </c:if>
                    </c:forEach>
                </div>
            </div>
        ...
        <div class="mdl-cell mdl-cell--12-col mdl-card mdl-shadow--2dp">
            <div class="mdl-grid titsonfire-card-content">
                ...
                <div class="mdl-cell mdl-cell--12-col mdl-typography--headline">${labelPrice}
                    &#160;<b>${commodity.branches[0].price} ${commodity.branches[0].currency}</b></div>
